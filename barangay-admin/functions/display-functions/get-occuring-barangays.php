...
if($query_click == "clicked")
{
    ...
    if (count($conditions) > 0) {
      $sql .= " WHERE `date` BETWEEN '$date_range_from' AND '$date_range_to' AND " . implode(' AND ', $conditions) . " AND `disease_name` = '$all_diseases_that_occured'
      GROUP BY date, barangay_name ORDER BY `date` DESC ";
    }
    else
    {
        $sql .= " WHERE  `date` BETWEEN '$date_range_from' AND '$date_range_to' AND `disease_name` = '$all_diseases_that_occured' AND `b`.`id` = '$barangay_name'
        GROUP BY date, barangay_name ORDER BY `date` DESC ";
    }
}
else
{
    ...
    LEFT JOIN diseases d ON (hp.disease_id = d.id) 
    LEFT JOIN barangays b ON (r.barangay_id = b.id) 
    WHERE  `date` BETWEEN '$current_year_from' AND '$current_year_to' AND `disease_name` = '$all_diseases_that_occured' AND `b`.`id` = '$barangay_name'
    GROUP BY date, barangay_name ORDER BY `date` DESC ";
}
