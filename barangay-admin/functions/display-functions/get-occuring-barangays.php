...
else
{
    ...
    ON (`hp`.`disease_id` = `d`.`id`) LEFT JOIN `barangays` AS `b` ON (`r`.`barangay_id` = `b`.`id`) WHERE `b`.`id` = '$barangay_name' AND  `date` BETWEEN '$current_year_from' AND '$current_year_to' && `disease_name` = '$all_diseases_that_occured' ORDER BY `date` DESC";
}
...
if ($result->num_rows > 0)
{
    while($row = $result->fetch_assoc())
    {
        ...
        else
        {
            ...
            ON (`hp`.`disease_id` = `d`.`id`) LEFT JOIN `barangays` AS `b` ON (`r`.`barangay_id` = `b`.`id`) WHERE `b`.`id` = '$barangay_name' AND  `date` = '$create_date' && `disease_name` = '$all_diseases_that_occured' && `barangay_name`= '$_barangay_namme' ORDER BY `date` DESC";
        }
