...
else
{
    ...
    ON (`hp`.`disease_id` = `d`.`id`) LEFT JOIN `barangays` AS `b` ON (`r`.`barangay_id` = `b`.`id`) WHERE `b`.`id`='$barangay_name' AND  `date` = '$all_dates' ORDER BY `disease_name` ASC";
}
...
if ($result->num_rows > 0)
{
    while($row = $result->fetch_assoc())
    {
        ...
        else
        {
            ...
            ON (`hp`.`disease_id` = `d`.`id`) LEFT JOIN `barangays` AS `b` ON (`r`.`barangay_id` = `b`.`id`) WHERE `b`.`id`='$barangay_name' AND  `date` = '$all_dates' && `barangay_name`= '$_barangay_namme' && `disease_name`= '$_diseases_name' ORDER BY `disease_name` ASC";
        }
