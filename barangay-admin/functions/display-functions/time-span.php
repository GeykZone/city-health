...
<?php
    ...
    else
    {
        ...
        ON (`hp`.`disease_id` = `d`.`id`) LEFT JOIN `barangays` AS `b` ON (`r`.`barangay_id` = `b`.`id`) WHERE `b`.`id`='$barangay_name' AND  `date` BETWEEN '$current_year_from' AND '$current_year_to' ORDER BY `date` ASC";
    }
    ...
    while($row = $result->fetch_assoc()) {
        ...
        else
        {
            ...
            ON (`hp`.`disease_id` = `d`.`id`) LEFT JOIN `barangays` AS `b` ON (`r`.`barangay_id` = `b`.`id`) WHERE `b`.`id`='$barangay_name' AND  `date` = '$date' ORDER BY `date` ASC";
        }
