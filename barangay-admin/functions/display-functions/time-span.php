...
<?php
    ...
    if($query_click == "clicked")
    {
        ...
        if (count($conditions) > 0) {
          $sql .= " WHERE `date` BETWEEN '$date_range_from' AND '$date_range_to' AND " . implode(' AND ', $conditions) . " GROUP BY `date` ORDER BY `date` ASC ";
        }
        else
        {
            $sql .= " WHERE `date` BETWEEN '$date_range_from' AND '$date_range_to' AND `b`.`id`='$barangay_name' GROUP BY `date` ORDER BY `date` ASC ";
        }
    ...
    else
    {
        ...
        LEFT JOIN diseases d ON (hp.disease_id = d.id) 
        LEFT JOIN barangays b ON (r.barangay_id = b.id)
        WHERE  `date` BETWEEN '$current_year_from' AND '$current_year_to' AND `b`.`id`='$barangay_name' GROUP BY `date` ORDER BY `date` ASC ";
    }
