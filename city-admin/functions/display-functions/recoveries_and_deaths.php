...
<?php
    ...
    if($query_click == "clicked")
    {
        ...
        {
            $sql .= "`case_status` = '(Inactive)'  AND `recovery` IS NOT NULL AND `date_of_recovery` BETWEEN '$date_range_from' AND '$date_range_to' ORDER BY `date_of_recovery` ASC ";
        }
    }
    else
    {
    ...
    while($row = $result->fetch_assoc()) 
    {
        ...
        if($query_click == "clicked")
        {
            ...
            {
                $sql2 .= "`case_status` = '(Inactive)'  AND `recovery` IS NOT NULL AND `date_of_recovery` = '$date' ORDER BY `date_of_recovery` ASC ";
            }
        }
        else
        {
