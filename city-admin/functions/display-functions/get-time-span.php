...
if($query_click == "clicked")
{
    ...
    if (count($conditions) > 0) {
        $sql .= " WHERE  `date` = '$all_dates' && " . implode(' AND ', $conditions) . " GROUP BY disease_name, barangay_name ORDER BY `disease_name` ASC ";
    }
    else
    {
        $sql .= " WHERE  `date` = '$all_dates' GROUP BY disease_name, barangay_name ORDER BY `disease_name` ASC ";
    }
}
else
{
    ...
    LEFT JOIN diseases d ON (hp.disease_id = d.id) 
    LEFT JOIN barangays b ON (r.barangay_id = b.id)
    WHERE  `date` = '$all_dates' GROUP BY disease_name, barangay_name ORDER BY `disease_name` ASC";
}
