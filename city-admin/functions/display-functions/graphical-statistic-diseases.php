...
<?php
    ...
    if($query_click == "clicked")
    {
        ...
        {
            $sql .= " WHERE `date` BETWEEN '$date_range_from' AND '$date_range_to' GROUP BY d.disease_name ORDER BY d.disease_name ASC ";
        }
    }
    else
    {
        ...
        LEFT JOIN diseases d ON (hp.disease_id = d.id) 
        LEFT JOIN barangays b ON (r.barangay_id = b.id)
        WHERE  `date` BETWEEN '$current_year_from' AND '$current_year_to' GROUP BY d.disease_name ORDER BY d.disease_name ASC ";
    }
