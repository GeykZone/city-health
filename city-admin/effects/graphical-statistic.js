...
{
    $(document).attr("title", "HPCS | Barangay Health Stats");
...
function oneTip()
{
    ...
    var current_year_tooltip = $("#current_year_table")
...
$("#hpChart").addClass("rounded-4 p-3 border-0 shadow-sm bg-light bg-opacity-50")
  ...
  var barangay_index = urlParams.get("index")

if (barangay_index)
{
  var current_index = barangay_index;
  title_barangay = x_value[current_index];
  $("#details_title").text("Barangay "+title_barangay+"")
  var all_diseases_that_occured = xValues[current_index];
  var display_diseases_that_occured = "";
  ...
  var modifiedSentence = modifiedWord.charAt(0).toUpperCase() + modifiedWord.slice(1);//make  first letter capital
    ...
    "buttons": [
      {
          ...
          {     
            ...
            mergedData += 'Barangay Health Stats\n';
      ...
      { 
          ...
          action: function ( e, dt, node, config ) {
              ...
              [],
              [{v: 'Barangay Health Stats', s: {font: {bold: true}, alignment: {horizontal: 'center'}}}],
              [],
              [{v: 'Barangay Name', s: {font: {bold: true}}}, {v: 'Total Health Cases', s: {font: {bold: true}, alignment: {horizontal: 'right'}}}]
              ]);
              ...
              var fileName = 'Barangay Health Stats ' + currentDate + '.xlsx';
    ...
    table.buttons().container().appendTo('#barangay_health_statistic_wrapper .col-md-6:eq(0)');
      ...
      $('#barangay_health_statistic #th_1 td').each(function () {
        ...
        {
          $(this).html('<input style="width: 180px;" type="text" class="form-control text-start table_search rounded-1 shadow-sm py-0"  placeholder="'+title+'" aria-controls="hp_table">');
        }
    ...
    $(".dt-buttons").addClass("col-lg-2 col-md-12 mb-2"); 
    $(".dt-buttons").removeClass("flex-wrap");
  
    $(".dataTables_length").detach().appendTo('#buttons')
    $(".dataTables_length").addClass("col-lg-10 text-lg-end text-center text-md-center text-sm-center col-md-12 mb-2");
...
$("#current_year_table").click(function()
{
